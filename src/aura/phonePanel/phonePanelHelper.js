...
({
                    ...
                    if (cmp.isValid() && cmp.get('v.presence') != 'Unavailable') {
                        sforce.opencti.getCallCenterSettings({
                            callback:function (result) {
                                let settings = result.returnValue;
                                let username = settings['/reqUpsellSettings/reqProviderAccount']
                                let password = settings['/reqUpsellSettings/reqProviderPassword']
                                let campaign = settings['/reqUpsellSettings/reqProviderCampaign']
                                let toNumber = payload.number
                                console.log(username)
                                console.log(password)
                                console.log(toNumber)
                                const xhr = new XMLHttpRequest();
                                const url = 'https://upsell.frd.cloud/api/ahx';
                                const data = { unencryptedData: { userName: username, password: password } };
                                ...
                                xhr.onreadystatechange = function() {
                                    if (xhr.readyState === 4 && xhr.status === 200) {
                                        ...
                                        const tempUrl = "https://upsell.frd.cloud/login?token="+token+"&camp="+campaign+"&number="+toNumber;
                                        console.log(tempUrl)
                                        
                                        // alert("オペレータ画面を確認してください。")
                                        if (!w || w.closed) {
                                            w = window.open(tempUrl);
                                          } else {
                                            console.log('window is already opened');
                                            w.location.replace(tempUrl)
                                            alert("オペレータ画面を確認してください。")
                                        }
                                    }
                                };
    ...
    updatePresence : function(cmp, event, helper) {
        ...
        var newMessage = '電話番号をクリックしてください。';
